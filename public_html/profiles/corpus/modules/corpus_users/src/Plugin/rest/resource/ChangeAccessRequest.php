...
namespace Drupal\corpus_users\Plugin\rest\resource;
 ...
 *
 * @RestResource(
 *   id = "corpus_users_request_access_level_change",
...
class ChangeAccessRequest extends ResourceBase {
  ...
  public static function create(ContainerInterface $container, array $configuration, $plugin_id, $plugin_definition) {
    return new static(
      ...
      $container->get('logger.factory')->get('corpus_users'),
  ...
  public function post($data) {
    ...
    $config = \Drupal::config('corpus_users.settings');
    if (!$config->get('on')) {
    ...
    $name = $full_name ?? $user->getDisplayName();
    $email = $user->getEmail();
    ...
    $admin_email = $this->getAdminEmailText($name, $data['role'], $data['description'], $current_roles, $user);
  ...
  public function getAdminEmailText($name, $requested_role, $justification, $current_roles, $user) {
    $body = [];
    $body[] = 'The user ' . $name . ' has requested an access level change.';
    $body[] = 'USERNAME: ' . $user->getDisplayName();
